...
(function (factory) {
  ...
  var SwimLaneLayout = /** @class */ (function (_super) {
    ...
    Object.defineProperty(SwimLaneLayout.prototype, 'laneProperty', {
      ...
      set: function (val) {
        if (typeof val !== 'string' && typeof val !== 'function')
          throw new Error(
            'new value for SwimLaneLayout.laneProperty must be a property name, not: ' +
              val,
          );
    ...
    Object.defineProperty(SwimLaneLayout.prototype, 'laneNames', {
      ...
      set: function (val) {
        if (!Array.isArray(val))
          throw new Error(
            'new value for SwimLaneLayout.laneNames must be an Array, not: ' +
              val,
          );
    ...
    Object.defineProperty(SwimLaneLayout.prototype, 'laneComparer', {
      ...
      set: function (val) {
        if (typeof val !== 'function')
          throw new Error(
            'new value for SwimLaneLayout.laneComparer must be a function, not: ' +
              val,
          );
    ...
    Object.defineProperty(SwimLaneLayout.prototype, 'laneSpacing', {
      ...
      set: function (val) {
        if (typeof val !== 'number')
          throw new Error(
            'new value for SwimLaneLayout.laneSpacing must be a number, not: ' +
              val,
          );
    ...
    SwimLaneLayout.prototype.nodeMinLayerSpace = function (v, topleft) {
      ...
      var edges = this.countEdgesForDirection(
        v,
    ...
    SwimLaneLayout.prototype.countEdgesForDirection = function (
      vertex,
      topleft,
    ) {
      var c = 0;
    ...
    SwimLaneLayout.prototype.setupLanes = function () {
      ...
      for (var i = 0; i <= this.maxLayer; i++) {
        this._layers[i].sort(function (a, b) {
      ...
      var _loop_1 = function (i) {
        ...
        if (arr) {
          ...
          for (var j = 0; j < arr.length; j++) {
            var v = arr[j];
            var w =
              this_1.nodeMinColumnSpace(v, true) +
              1 +
              this_1.nodeMinColumnSpace(v, false);
      ...
      for (var i = 0; i < this._layers.length; i++) {
        ...
        for (var j = 0; j < arr.length; j++) {
          ...
          if (l && prevlane !== l) {
            ...
            var z =
              this.nodeMinColumnSpace(v, true) +
              1 +
              this.nodeMinColumnSpace(v, false);
            var k = j + 1;
            while (k < arr.length && this.findLane(arr[k]) === l) {
              var vz = arr[k];
              z +=
                this.nodeMinColumnSpace(vz, true) +
                1 +
                this.nodeMinColumnSpace(vz, false);
              k++;
            }
